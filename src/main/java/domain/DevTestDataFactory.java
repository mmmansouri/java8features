...
public class DevTestDataFactory {
  ...
  private static Account getAccount(int initialBalance) {
    ...
    account.setAccountOwnerName("Mr Smith");
    return account;
  }
  ...
  private static Account getAccountWithTransactions(int initialBalance) {
    ...
    Transaction transaction1 = Transaction.builder().transactionType(TransactionType.DEPOSIT)
        .amount(10).date(new Date()).balance(account.getBalance() + 10).build();
    operations.add(transaction1);
    Transaction transaction2 = Transaction.builder().transactionType(TransactionType.WITHDRAWAL)
        .amount(20).date(new Date()).balance(account.getBalance() - 20).build();
    operations.add(transaction2);
    Transaction transaction3 = Transaction.builder().transactionType(TransactionType.DEPOSIT)
        .amount(11).date(new Date()).balance(account.getBalance() + 11).build();
    operations.add(transaction3);
    Transaction transaction4 = Transaction.builder().transactionType(TransactionType.DEPOSIT)
        .amount(18).date(new Date()).balance(account.getBalance() + 18).build();
    operations.add(transaction4);
    Transaction transaction5 = Transaction.builder().transactionType(TransactionType.WITHDRAWAL)
        .amount(12).date(new Date()).balance(account.getBalance() - 12).build();
